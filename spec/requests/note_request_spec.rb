...
RSpec.describe 'Notes API', type: :request do
  ...
  context 'listing notes api' do
    ...
    it 'renders notes in correct json structure for GET /notes' do
      ...
      note2 = @user.notes.create(content: 'hey2', collapsed: false, path: "/#{note1.id}/",
                                 order: 0)
      note3 = @user.notes.create(content: 'hey3', collapsed: false, path: "/#{note1.id}/",
                                 order: 1)
      ...
      expect(json.map { |i| i.deep_symbolize_keys }).to eq(expected_response)
    end
  end
  ...
  context 'creating notes through transactions api' do
    it 'creates a new note correctly for a "created" transaction' do
      ...
      post '/api/notes/process_transactions',
           headers: { 'Authorization' => "Bearer #{jwt}", 'Content-Type': 'application/json' }, params: transactions.to_json
