class Note
  ...
  def self.add_new_child_tree(user_id, parent_id, index, note_id, fields)
    note_path = "/"
    ...
    correct_orders_for_siblings(note_path, index, false)
  ...
  def self.correct_orders_for_siblings(path, index, move_left)
    siblings = Note.where(path: path)
    siblings.each do |sibling|
      next unless sibling

      if sibling.order > index
        sibling.update(order: move_left ? sibling.order - 1 : sibling.order + 1)
      end
    end
  end
end
